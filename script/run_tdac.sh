...
# =========================
# 3) Convert demux BAM -> FASTQ.gz
# =========================
echo "[*] Converting demultiplexed BAMs to FASTQ.gz..."
parallel -j 8 'mamba run -n nano samtools fastq -@ 4 {} | gzip -c > '${FASTQ_DIR}'/{/.}.fastq.gz' ::: "${DEMUX_DIR}"/*.bam

# =========================
# 4) Build index of demux file -> barcode number
# =========================
...
# =========================
# 5) Merge per-sample across barcodes
# =========================
echo "[*] Merging FASTQ files per sample..."

# Process each unique sample
cut -d',' -f1 "${SAMPLE_CSV}" | tail -n +2 | sort | uniq | while read -r sample; do
  [[ -z "${sample}" ]] && continue
  
  # Get all barcodes for this sample
  mapfile -t barcode_list < <(awk -F',' -v s="${sample}" '$1==s {print $2}' "${SAMPLE_CSV}")
  ...
  if (( ${#barcode_list[@]} == 1 )); then
    ...
    if [[ -n "${demux_bam}" ]]; then
      ...
      if [[ -f "${src_fastq}" ]]; then
        echo "[*] Linking ${sample} (1 barcode) -> ${out_fq}"
        ln -sf "$(realpath "${src_fastq}")" "${out_fq}"
      else
        echo "[!] Warning: Source FASTQ not found: ${src_fastq}" >&2
      fi
    else
      echo "[!] Warning: No demux BAM for barcode ${nb}" >&2
    fi
    
  else
    # Multiple barcodes - merge with proper gzip handling
    fq_files=()
...
# =========================
# 6) Create rename table (points to merged files)
# =========================
echo -e "Sample\tBarcode\tBarcodeNum\tFASTQ" > "${RENAME_TABLE}"

echo "[*] Building rename table with merged FASTQ paths..."
tail -n +2 "${SAMPLE_CSV}" | while IFS=',' read -r sample nb; do
